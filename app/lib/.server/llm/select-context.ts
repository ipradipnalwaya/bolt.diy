...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyWebBanaoActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyWebBanaoActions(content);

      content = content.replace(/<div class=\\"__webbanaoThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`WebBanao failed to select files`);
  }
