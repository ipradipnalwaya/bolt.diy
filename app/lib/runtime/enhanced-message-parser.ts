...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<webbanaoArtifact') || input.includes('</webbanaoArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<webbanaoArtifact id="${artifactId}" title="${title}" type="bundled">
<webbanaoAction type="file" filePath="${filePath}">
${content}
</webbanaoAction>
</webbanaoArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<webbanaoArtifact id="${artifactId}" title="Shell Command" type="shell">
