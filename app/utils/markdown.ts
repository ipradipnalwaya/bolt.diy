  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__webbanaoThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__webbanaoArtifact__', '__webbanaoThought__', '__webbanaoQuickAction', '__webbanaoSelectedElement__'],

      // ['className', '__webbanaoThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__webbanaoArtifact__', '__webbanaoThought__', '__webbanaoQuickAction'],
    ],
  },
  strip: [],
};
