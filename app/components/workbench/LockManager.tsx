...
export function LockManager() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Controls */}
      <div className="flex items-center gap-1 px-2 py-1 border-b border-webbanao-elements-borderColor">
        {/* Search Input */}
        <div className="relative flex-1">
          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-webbanao-elements-textTertiary i-ph:magnifying-glass text-xs pointer-events-none" />
          <input
            type="text"
            placeholder="Search..."
            className="w-full text-xs pl-6 pr-2 py-0.5 h-6 bg-webbanao-elements-background-depth-2 text-webbanao-elements-textPrimary rounded border border-webbanao-elements-borderColor focus:outline-none"
            value={searchTerm}
        ...
        {/* Filter Select */}
        <select
          className="text-xs px-1 py-0.5 h-6 bg-webbanao-elements-background-depth-2 text-webbanao-elements-textPrimary rounded border border-webbanao-elements-borderColor focus:outline-none"
          value={filter}
      ...
      <div className="flex items-center justify-between px-2 py-1 text-xs text-webbanao-elements-textSecondary">
        <div>
          <Checkbox
            checked={selectAllCheckedState}
            onCheckedChange={handleSelectAll}
            className="w-3 h-3 rounded border-webbanao-elements-borderColor mr-2"
        ...
        {selectedItems.size > 0 && (
          <button
            className="ml-auto px-2 py-0.5 rounded bg-webbanao-elements-button-secondary-background hover:bg-webbanao-elements-button-secondary-backgroundHover text-webbanao-elements-button-secondary-text text-xs flex items-center gap-1"
      ...
      <div className="flex-1 overflow-auto modern-scrollbar px-1 py-1">
        {filteredAndSortedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-webbanao-elements-textTertiary text-xs gap-2">
            ...
            {filteredAndSortedItems.map((item) => (
              <li
                ...
                className={classNames(
                  'text-webbanao-elements-textTertiary flex items-center gap-2 px-2 py-1 rounded hover:bg-webbanao-elements-background-depth-2 transition-colors group',
                  selectedItems.has(item.path) ? 'bg-webbanao-elements-background-depth-2' : '',
                )}
              >
                <Checkbox
                  checked={selectedItems.has(item.path)}
                  onCheckedChange={() => handleSelectItem(item.path)}
                  className="w-3 h-3 rounded border-webbanao-elements-borderColor"
                  aria-labelledby={`item-label-${item.path}`} // For accessibility
                />
                <span
                  className={classNames(
                    'shrink-0 text-webbanao-elements-textTertiary text-xs',
                ...
                ></span>
                <button
                  className="flex items-center px-1 py-0.5 text-xs rounded bg-transparent hover:bg-webbanao-elements-background-depth-3"
                  onClick={() => {
                    if (item.type === 'file') {
      ...
      {/* Footer */}
      <div className="px-2 py-1 border-t border-webbanao-elements-borderColor bg-webbanao-elements-background-depth-2 text-xs text-webbanao-elements-textTertiary flex justify-between items-center">
        <div>
