...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      webbanaoVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.webbanaoVersion) {
      body += `- webbanao.diy: ${data.environmentInfo.webbanaoVersion}\n`;
    }
  ...
  body += '---\n*Submitted via webbanao.diy bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/webbanao.diy';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'webbanao.diy-bug-reporter',
    });
