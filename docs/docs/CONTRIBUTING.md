...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make **webbanao.diy** a better tool for developers worldwide. 💡
...
- Clone the repository:
  ```bash
  git clone https://github.com/stackblitz-labs/webbanao.diy.git
  ```
...
```bash
docker run -p 5173:5173 --env-file .env.local webbanao-ai:development
```
...
```bash
docker build . --target webbanao-ai-production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local webbanao-ai:production
```
